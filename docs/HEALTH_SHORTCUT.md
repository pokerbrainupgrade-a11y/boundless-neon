...
BOUNDLESS NEON is a web app, so it cannot read HealthKit directly. This iOS Shortcut bridges the gap through the clipboard:

1. At the end of a session the app shows **Pull HR from Health**. Tapping it copies `BNEON|<sessionId>|<startISO>|<endISO>` to the clipboard and opens `shortcuts://run-shortcut?name=Boundless%20HR`.
2. The Shortcut reads the clipboard, finds heart-rate samples in that window, computes the average and maximum, copies `BNEON-HR|<sessionId>|<avg>|<max>` to the clipboard, and returns to the app.
...
| # | Action | Configure |
|---|---|---|
| 1 | **Get Clipboard** | No options. Output is the request text `BNEON\|<sessionId>\|<start>\|<end>`. |
...
| 11 | **Text** | Type exactly: `BNEON-HR|` then insert variable `SessionID`, then `|`, insert `AvgHR`, then `|`, insert `MaxHR`. No spaces. Result looks like `BNEON-HR|A-20260921T140000|142|171`. |
| 12 | **Copy to Clipboard** | Input: the *Text* from step 11. |
| 13 | **Open URLs** | URL: `https://pokerbrainupgrade-a11y.github.io/boundless-neon/`. If the app is installed on the Home Screen, you can use **Open App → Boundless** instead so it returns to the installed app. |

Tap **Done**. Run the Shortcut once by hand from the Shortcuts app so it can request Health and clipboard permissions.

## Using it

1. Finish a session and tap **Pull HR from Health** on the journal form.
