  ...
  return (
    <div style="display:flex;flex-direction:column;align-items:center;gap:6px" data-testid="breath-ring" data-phase={phase}>
      <svg width="200" height="200" viewBox="0 0 200 200" aria-label={`Breathing pacer: ${phase}`} role="img">
        ...
        <circle cx="100" cy="100" r={r} fill={phase === 'inhale' ? 'rgba(0,229,255,0.18)' : 'rgba(255,46,151,0.18)'} stroke={phase === 'inhale' ? 'var(--cyan)' : 'var(--pink)'} stroke-width="4" />
      </svg>
      <div class="timer-state" style="font-size:1.3rem">{phase === 'inhale' ? 'INHALE · RIBS WIDE' : 'EXHALE · STAY TALL'}</div>
    </div>
  );
}

/** Box breathing 4-4-4-4: a dot travels around a square. */
  ...
  return (
    <div style="display:flex;flex-direction:column;align-items:center;gap:6px" data-testid="box-breath" data-side={side}>
      <svg width="200" height="200" viewBox="0 0 200 200" role="img" aria-label={`Box breathing: ${labels[side]}`}>
        <rect x={o} y={o} width={S} height={S} rx="10" fill="none" stroke="var(--surface-2)" stroke-width="4" />
        <rect x={o} y={o} width={S} height={S} rx="10" fill="none" stroke="var(--purple)" stroke-width="4" stroke-dasharray={`${S * 4}`} stroke-dashoffset={`${S * 4 - (side * S + p * S)}`} opacity="0.8" />
        <circle cx={pos[0]} cy={pos[1]} r="9" fill="var(--cyan)" />
        <text x="100" y="108" text-anchor="middle" font-family="var(--font-timer)" font-size="30" fill="var(--text)">{count}</text>
      </svg>
