...
/** Hand-rolled SVG bar/line chart. Prenatal points are marked with a hollow diamond. */
export function Chart({ data }: { data: ChartData }) {
  ...
  const anyPrenatal = series.some((s) => s.points.some((p) => p.prenatal));
  return (
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" role="img" aria-label={`${data.title}: ${empty ? 'no data yet' : series.map((s) => `${s.name} ${s.points.map((p) => `${p.x} ${p.y ?? '—'}`).join(', ')}`).join('; ')}`}>
        {ticks.map((t) => (
          <g key={t}>
            ...
            <text x={PL - 4} y={yPix(t) + 4} text-anchor="end" font-size="9" fill="var(--muted)">{Math.round(t)}</text>
          </g>
        ))}
        {xs.map((x, i) => <text key={x} x={PL + slot * i + slot / 2} y={H - PB + 14} text-anchor="middle" font-size="9" fill="var(--muted)">{x}</text>)}
        ...
        {data.kind === 'bar' && series.map((s, si) => s.points.map((p) => {
          ...
          return (
            <g key={s.name + p.x}>
              <rect x={x} y={y} width={bw} height={PT + ih - y} rx="2" fill={s.color} opacity={p.prenatal ? 0.6 : 0.9} />
              {p.prenatal && <path d={`M${x + bw / 2} ${y - 8} l4 4 -4 4 -4 -4z`} fill="none" stroke="var(--purple-light)" stroke-width="1.5" />}
            </g>
          );
        }))}
        {data.kind === 'line' && series.map((s) => {
          const pts = s.points.filter((p) => p.y !== null).map((p) => [PL + slot * xi(p.x) + slot / 2, yPix(p.y!), !!p.prenatal] as const);
          if (!pts.length) return null;
          return (
            <g key={s.name}>
              <polyline points={pts.map((p) => `${p[0]},${p[1]}`).join(' ')} fill="none" stroke={s.color} stroke-width="2" stroke-linejoin="round" />
              {pts.map((p, i) => p[2] ? <path key={i} d={`M${p[0]} ${p[1] - 5} l5 5 -5 5 -5 -5z`} fill="var(--bg)" stroke="var(--purple-light)" stroke-width="1.5" /> : <circle key={i} cx={p[0]} cy={p[1]} r="3" fill={s.color} />)}
            </g>
          );
        })}
      </svg>
      <div class="row wrap" style="gap:6px">
        {series.slice(0, 8).map((s) => <span key={s.name} class="small muted"><span style={`display:inline-block;width:10px;height:10px;border-radius:2px;background:${s.color};margin-right:4px;vertical-align:middle`} />{s.name}</span>)}
        {anyPrenatal && <span class="small" style="color:var(--purple-light)">◇ prenatal session (not a 1:1 comparison)</span>}
      </div>
    </figure>
  );
}
