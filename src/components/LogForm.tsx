...
import { saveLog, ctx } from '@/lib/store';
...
export function LogForm({ session, original, draft, onSaved, onCancel, note }: LogFormProps) {
  ...
  const save = async () => {
    setSaving(true);
    const id = await saveLog({ ...log, prenatal: ctx.value.on, trimester: ctx.value.on ? ctx.value.trimester : undefined });
    onSaved(id);
  };
  ...
  return (
    <main class="screen" data-testid="log-form">
      <div class="section-h"><h1>Journal it</h1></div>
      <div class="muted small">{session.name}{session.id !== original.id ? ` (replaces ${original.name})` : ''} · Day {log.dayN}</div>
      ...
      {preset === 'tabata' && (
        <div class="card stack">
          ...
          <div class="row between"><span>Total <strong>{String(data.total ?? '—')}</strong></span><span>Round-8 drop-off <strong>{data.dropoff !== undefined ? `${data.dropoff}%` : '—'}</strong></span></div>
        </div>
      )}
      ...
      {preset === 'vo2' && (
        <div class="card stack">
          ...
          {max && !(data.hideHr === true) && <div class="muted small">Target band {Math.round(max * 0.87)}–{Math.round(max * 0.97)} bpm</div>}
          ...
          {program.superSlowPatterns.map((p, i) => {
            ...
            return (
              <div key={p.id} class="stack" style="gap:6px">
                <div class="row between"><strong>{p.name}</strong>{flag && <span class="chip chip-yellow">{flag}</span>}</div>
      ...
      <div class="card stack">
        ...
        <div class="row wrap" role="radiogroup" aria-label="RPE 1 to 10">
          {Array.from({ length: 10 }, (_, i) => i + 1).map((n) => (
            <button key={n} type="button" role="radio" aria-checked={log.rpe === n} class={`btn ${log.rpe === n ? 'btn-primary' : ''}`} style="min-width:48px;padding:0" onClick={() => setLog({ ...log, rpe: n })}>{n}</button>
          ))}
        </div>
        {ctx.value.on && <div class="muted small">Prenatal: aim for RPE 6–7. You should be able to talk in short sentences.</div>}
      </div>
      ...
      <div class="card stack">
        ...
        <div class="row">
          <button type="button" class="btn btn-cyan grow" onClick={pullHr}>Pull HR from Health</button>
        ...
        <details>
          ...
          <div class="row" style="margin-top:6px">
            <input class="input grow" data-testid="hr-paste-box" placeholder="BNEON-HR|…" value={pasteBox} onInput={(e) => setPasteBox((e.target as HTMLInputElement).value)} />
      ...
      <div class="row">
        ...
        <button type="button" class="btn btn-lime btn-lg grow" data-testid="save-log" disabled={saving} onClick={save}>SAVE</button>
      </div>
    </main>
  );
}
