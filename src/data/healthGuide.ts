...
export const HEALTH_GUIDE = {
  ...
  steps: [
    ...
    { action: 'Add action: Get Clipboard.', detail: 'Output: the text "BNEON|<sessionId>|<startISO>|<endISO>".' },
    ...
    { action: 'Add action: Text.', detail: 'Content exactly: BNEON-HR|SessionID|AvgHR|MaxHR (insert the three variables; keep the pipes, no spaces).' },
    { action: 'Add action: Copy to Clipboard.', detail: 'Input: the Text from the previous step.' },
    { action: 'Add action: Open URLs.', detail: 'URL: https://pokerbrainupgrade-a11y.github.io/boundless-neon/ (if the app was installed to the Home Screen, use Open App → Boundless instead, which is available once the app is installed).' },
