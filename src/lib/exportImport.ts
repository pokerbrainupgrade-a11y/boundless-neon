...
const BlockSchema = z.object({
  ...
  prenatal: z.boolean().default(false),
  createdAt: z.string(),
...
const LogSchema = z.object({
  ...
  prenatal: z.boolean(),
  trimester: z.enum(['T1', 'T2', 'T3']).optional(),
  completed: z.boolean(),
...
export function exportFilename(date = new Date()): string {
  return `boundless-neon-backup-${date.toISOString().slice(0, 10)}.json`;
}
...
export function parseImport(text: string): { ok: true; data: ExportFile } | { ok: false; error: string } {
  ...
  if (!r.success) return { ok: false, error: 'Not a Boundless backup: ' + r.error.issues.slice(0, 3).map((i) => `${i.path.join('.')} ${i.message}`).join('; ') };
