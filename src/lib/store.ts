...
export const ctx = computed(() => prenatalContext(settings.value, todayYmd.value));
...
export async function startBlock(startDate: string): Promise<Block> {
  ...
  const b: Block = { n: (prev?.n ?? 0) + 1, startDate, shift: 0, prenatal: settings.value.prenatalMode, createdAt: new Date().toISOString() };
