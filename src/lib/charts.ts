...
const COLORS = ['var(--pink)', 'var(--cyan)', 'var(--lime)', 'var(--yellow)', 'var(--purple-light)'];
...
export function groups(logs: SessionLog[], blocks: Block[], mode: ChartMode, blockId: number | null): { name: string; logs: SessionLog[] }[] {
  ...
  return blocks.map((b) => ({ name: `Season ${String(b.n).padStart(2, '0')}`, logs: logs.filter((l) => l.blockId === b.id) }));
}
...
export function buildCharts(logs: SessionLog[], blocks: Block[], vitals: Vital[], mode: ChartMode, blockId: number | null): ChartData[] {
  ...
  out.push({
    ...
    series: gs.map((g, i) => ({ name: g.name, color: COLORS[i % COLORS.length]!, points: done(g.logs, 'A').map((l) => ({ x: `D${l.day}`, y: n(l.data.total), prenatal: l.prenatal, note: String(l.data.movement ?? '') })) })),
  });
  ...
  out.push({
    ...
    series: gs.flatMap((g, i) => done(g.logs, 'A').map((l, k) => ({ name: `${g.name} D${l.day}`, color: COLORS[(i * 3 + k) % COLORS.length]!, points: ((l.data.rounds as (number | null)[]) ?? []).slice(0, 8).map((r, j) => ({ x: `R${j + 1}`, y: n(r), prenatal: l.prenatal })) }))),
  });
  ...
  for (const [key, title, unit] of [['seconds', 'Super-slow seconds to failure', 's'], ['load', 'Super-slow load per lift', 'load']] as const) {
    out.push({
      ...
      series: gs.map((g, i) => {
        ...
        return { name: g.name, color: COLORS[i % COLORS.length]!, points: patterns.map((p) => ({ x: pname[p]!, y: n(lifts.find((x) => x.pattern === p)?.[key]), prenatal: l?.prenatal })) };
      }),
    });
  }
  ...
  out.push({
    ...
    series: gs.flatMap((g, i) => done(g.logs, 'H').map((l) => ({ name: g.name, color: COLORS[i % COLORS.length]!, points: ((l.data.rounds as (number | null)[]) ?? []).slice(0, 5).map((r, j) => ({ x: `R${j + 1}`, y: n(r), prenatal: l.prenatal })) }))),
  });
  // Sauna minutes (or cool stretch in prenatal)
  out.push({
    id: 'sauna', title: 'Sauna minutes', unit: 'min', kind: 'bar',
    series: gs.map((g, i) => ({ name: g.name, color: COLORS[i % COLORS.length]!, points: g.logs.filter((l) => l.completed && (l.sessionId === 'I')).map((l) => ({ x: `D${l.day}`, y: n(l.data.minutes), prenatal: l.prenatal, note: l.ranSessionId !== 'I' ? 'cool stretch' : undefined })) })),
  });
  // Stamina duration / HR
  out.push({
    ...
    series: gs.flatMap((g, i) => {
      ...
      return [
        { name: `${g.name} min`, color: COLORS[i % COLORS.length]!, points: ls.map((l) => ({ x: `D${l.day}`, y: n(l.data.durationMin), prenatal: l.prenatal })) },
        { name: `${g.name} HR`, color: COLORS[(i + 2) % COLORS.length]!, points: ls.map((l) => ({ x: `D${l.day}`, y: n(l.data.avgHr) ?? n(l.hr?.avg), prenatal: l.prenatal })) },
      ];
    }),
  });
  // Cold dose
  out.push({
    ...
    series: gs.flatMap((g, i) => {
      ...
      return [
        ...
        { name: `${g.name} immersion`, color: COLORS[(i + 1) % COLORS.length]!, points: imm.map((l) => ({ x: `D${l.day} min`, y: n(l.data.minutes), prenatal: l.prenatal })) },
        { name: `${g.name} water °F`, color: COLORS[(i + 3) % COLORS.length]!, points: imm.map((l) => ({ x: `D${l.day} °F`, y: n(l.data.waterF), prenatal: l.prenatal })) },
      ];
    }),
  });
