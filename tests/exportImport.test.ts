...
async function seed(d: BoundlessDB) {
  const blockId = (await d.blocks.add({ n: 1, startDate: '2026-09-21', shift: 0, prenatal: true, createdAt: '2026-09-20T12:00:00Z' })) as number;
  await d.logs.bulkAdd([
    { blockId, dayN: 1, week: 1, day: 1, slot: 'main', sessionId: 'A', ranSessionId: 'A', variant: 'bike', startedAt: '2026-09-21T14:00:00Z', endedAt: '2026-09-21T14:05:00Z', rpe: 7, prenatal: true, trimester: 'T1', completed: true, data: { rounds: [12, 12, 11, 11, 10, 10, 9, 9], total: 84, dropoff: 25 } },
    { blockId, dayN: 4, week: 1, day: 4, slot: 'main', sessionId: 'F', ranSessionId: 'F', startedAt: '2026-09-24T14:00:00Z', endedAt: '2026-09-24T14:25:00Z', prenatal: true, trimester: 'T1', completed: true, hr: { avg: 120, max: 150 }, data: { lifts: [{ pattern: 'upperPush', load: 60, seconds: 110, reps: 3 }] } },
  ]);
  ...
  await d.kv.put({ key: 'settings', value: { displayName: 'x', prenatalMode: true, age: 34 } });
}
...
describe('export → wipe → import round-trip', () => {
  ...
  it('rejects invalid files', () => {
    ...
    const r = parseImport(JSON.stringify({ app: 'boundless-neon', schemaVersion: 1, exportedAt: 'x', blocks: [], logs: [{ bad: true }], habits: [], vitals: [] }));
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error).toContain('Not a Boundless backup');
  });

  it('names the file by date', () => {
    expect(exportFilename(new Date('2026-09-16T20:00:00Z'))).toBe('boundless-neon-backup-2026-09-16.json');
  });
});
