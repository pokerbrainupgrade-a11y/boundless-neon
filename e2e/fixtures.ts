/** Seeded fake data for Progress and Journal tests. */
export function demoExport(startDate = '2026-09-21', prenatal = false) {
  const b1 = 1, b2 = 2;
  const log = (blockId: number, dayN: number, sessionId: string, data: Record<string, unknown>, extra: Record<string, unknown> = {}) => ({
    blockId, dayN, week: (dayN <= 7 ? 1 : 2) as 1 | 2, day: ((dayN - 1) % 7) + 1, slot: 'main' as const, sessionId, ranSessionId: sessionId,
    startedAt: new Date(Date.UTC(2026, 8, 20 + dayN, 14, 0)).toISOString(), endedAt: new Date(Date.UTC(2026, 8, 20 + dayN, 14, 30)).toISOString(),
    prenatal, trimester: prenatal ? ('T2' as const) : undefined, completed: true, data, ...extra,
  });
  ...
  return {
    app: 'boundless-neon' as const,
    schemaVersion: 1 as const,
    exportedAt: '2026-10-20T00:00:00.000Z',
    blocks: [
      { id: b1, n: 1, startDate, shift: 0, prenatal, createdAt: '2026-09-20T00:00:00.000Z', endedAt: '2026-10-05T00:00:00.000Z' },
      { id: b2, n: 2, startDate: '2026-10-12', shift: 0, prenatal, createdAt: '2026-10-11T00:00:00.000Z' },
    ],
    logs,
