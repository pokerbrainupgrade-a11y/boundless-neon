...
test.describe('schedule and Phoenix day math', () => {
  test('start date + N days → correct day across Phoenix midnight; rollover after day 14', async ({ page }) => {
    ...
    await setup(page, { prenatalMode: false, age: 34 }, '2026-09-21', '2026-09-24T06:30:00Z');
    ...
    await page.reload();
    await expect(page.getByText('Season 01 complete')).toBeVisible();
    await page.getByTestId('start-date-input').fill('2026-10-12');
    await page.getByTestId('start-block').click();
    await expect(page.getByText('Season 02 starts')).toBeVisible();
  ...
  test('shift remaining days +1 pushes the schedule', async ({ page }) => {
    await setup(page, { prenatalMode: false }, '2026-09-21', '2026-09-25T20:00:00Z');
    await page.goto('/boundless-neon/#/today');
    await expect(page.getByTestId('day-chip')).toHaveText('Day 05');
    await page.goto('/boundless-neon/#/schedule');
